...
--학생 테이블의 sname에 NOT NULL추가
--NOT NULL을 추가할 때는 ADD가 아니라 MODIFY를 사용
ALTER TABLE student
MODIFY sname CONSTRAINT student_sname_nn NOT NULL;
--학생 테이블에 외래키 추가
ALTER TABLE student
ADD CONSTRAINT student_cno_fk FOREIGN KEY(cno) REFERENCES class (cno); --클래스 테이블에 cno열을 참조
SELECT * FROM class;
SELECT * FROM student;
--제약조건을 추가할 때 이미 만들어진 테이블의 각 행의 데이터가 만족해야 제약조건이 추가된다.
UPDATE student SET cno = '01'
WHERE sno = '0416';

--제약조건의 삭제
--제약조건의 이름으로 삭제, CASCADE 옵션시에는 참조하는 외래키도 삭제
ALTER TABLE class
...
ALTER TABLE E_EMP
MODIFY (제작자 VARCHAR2(1)); -- 원래 (50) => (1) 사이즈로는 에러발생 안에 2바이트가 존재해서.

ALTER TABLE E_EMP
MODIFY (제작자 NUMBER); -- 이미 문자형 데이터가 있기때문에 숫자형으로 바뀔 수 없음.

SELECT * FROM E_EMP;

--NULL값인 경우에 입력된 값이 없었기 때문에 데이터 형식을 바꿀 수 있음.
ALTER TABLE E_EMP
...
ALTER TABLE E_EMP
MODIFY (제작자 NUMBER); --안됨 (?)

--열의 삭제
ALTER TABLE E_EMP
DROP COLUMN 제작자; --제작자 열 하나 삭제
--여러개의 열 삭제
ALTER TABLE E_EMP
DROP (GENDER, 커미션, 날짜);
...
--예제 employees 테이블의 job_id가 'ST_MAN'인 직원들을 새 테이블을 만들어 입력하라.
